...
void Logger::Title(const std::string& msg, char surroundingLine) {
	std::string mark(1, surroundingLine);
	std::string temp = "\n " + std::string(msg.size() + 4, surroundingLine);
	temp += "\n " + mark + std::string(msg.size() + 2, ' ') + mark;
	temp += "\n " + mark + " " + msg + " " + mark;
	temp += "\n " + mark + std::string(msg.size() + 2, ' ') + mark;
	temp += "\n " + std::string(msg.size() + 4, surroundingLine);
	write(temp);
